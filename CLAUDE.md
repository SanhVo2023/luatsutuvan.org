...
- **Design system** is bespoke Tailwind v4 `@theme` tokens in `src/app/globals.css` (teal-forward palette, 17px base / 1.2 modular scale, `--font-be-vietnam`). There is **no `@tailwindcss/typography`** — guide bodies use the hand-rolled `.guide-prose` class. Reuse the semantic tokens (`text-ink`, `bg-surface`, `text-teal`, `border-line`, `shadow-soft`, `bg-aurora`, `bg-guide-grid`) rather than raw hex.
