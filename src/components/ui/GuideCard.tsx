...
const ACCENT: Record<string, string> = {
  teal: 'from-teal-50 text-teal-700 ring-teal/15',
  sky: 'from-sky-50 text-sky ring-sky/20',
  gold: 'from-gold-soft text-gold ring-gold/20',
  success: 'from-[#eef9f0] text-success ring-success/20',
}

export function GuideCard({ guide }: { guide: Guide }) {
  const pillar = PILLAR_MAP[guide.pillar]
  const accent = ACCENT[pillar?.accent ?? 'teal']
  const href = `/${pillar.slug}/${guide.slug}/`
  return (
    <HoverZoom className="h-full">
      <Link
        href={href}
        className="group flex h-full flex-col rounded-3xl border border-line bg-white p-6 shadow-soft transition-all hover:border-teal/30 hover:shadow-lift"
      >
        ...
        <h3 className="mt-4 font-heading text-[1.18rem] font-semibold leading-snug text-ink transition-colors group-hover:text-teal-700">
          {guide.title}
        </h3>
