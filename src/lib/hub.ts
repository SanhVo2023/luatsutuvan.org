 ...
 *
 * NOTE: This site (luatsutuvan.org) is a hybrid — its 40 structured guides stay
 * on local markdown (src/lib/guides.ts); the hub drives shared boilerplate
 * (disclaimer, author byline) and receives contact submissions. `hubEnabled()`
 * gates the hub-backed reads so the site degrades cleanly to its local content.
 */
...
/** True when the hub integration is switched on for this deployment. */
export function hubEnabled(): boolean {
  return process.env.NEXT_PUBLIC_USE_HUB === '1' && !!process.env.HUB_API_URL
}
...
/**
 * POST /api/contact-submissions to the hub. Returns {ok,id} or {ok:false,error}.
 * Runs server-side (route handler). The GAS mirror is handled by the caller so
 * this stays a single-responsibility hub write.
 */
